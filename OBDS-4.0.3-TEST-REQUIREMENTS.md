# OBDS 4.0.3 Test and Runtime Requirements
...
OBDS 4.0.3 is a data and contract specification. A conforming implementation may be written in
any language. The normative artefacts are plain text:

- `OBDS-4.0.3.md` (normative specification);
...
- `OBDS-4.0.3-SCHEMA-INDEX.json` and `OBDS-4.0.3-CAPABILITY-REGISTRY.json`.
...
```bash
unzip OBDS-4.0.3-FINAL.zip && cd OBDS-4.0.3-FINAL
python -m venv .venv && .venv/bin/pip install -r requirements.txt
.venv/bin/python reference/run_all.py
.venv/bin/python reference/release-gate.py
```
