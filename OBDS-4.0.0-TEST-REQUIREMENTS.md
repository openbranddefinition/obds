# OBDS 4.0.0 Test and Runtime Requirements
...
The 4.0.0 closure cases exercise F1 path and symlink containment, F2 failed-generation isolation and explicit rollback, F3 resealed tampering of every governed projection slot, F4 malformed manifest structures and approval dates, and F5 persisted provider-failure records with no output release. Systemic hash and executor checks also cover the generation loader.

## 1. OBDS consumer requirements

**None imposed by the specification.**

OBDS 4.0.0 is a data and contract specification. A conforming implementation may be written in
any language. The normative artefacts are plain text:

- `OBDS-4.0.0.md` (normative specification);
...
- `OBDS-4.0.0-SCHEMA-INDEX.json` and `OBDS-4.0.0-CAPABILITY-REGISTRY.json`.
...
```bash
unzip OBDS-4.0.0-FINAL.zip && cd OBDS-4.0.0-FINAL
python -m venv .venv && .venv/bin/pip install -r requirements.txt
.venv/bin/python reference/run_all.py
.venv/bin/python reference/release-gate.py
```
...
```text
1158 passed, 0 failed, 0 skipped
```
