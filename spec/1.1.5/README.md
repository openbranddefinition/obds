...
artifactHash   sha256:193b760d887d5730615664f5ae95ccced0aa9375528ef5dcd2cdcdaea560fc3f
...
| [`OBDS-1.1.5-IMPLEMENTER-QUICKSTART.md`](OBDS-1.1.5-IMPLEMENTER-QUICKSTART.md) | five concepts, then the smallest conforming implementation |
| [`OBDS-1.1.5.md`](OBDS-1.1.5.md) | the normative specification |
...
| [`OBDS-1.1.5-TEST-RESULT.json`](OBDS-1.1.5-TEST-RESULT.json) | the verified conformance result |

Also: [`CONTRIBUTING.md`](CONTRIBUTING.md) for how to propose a change,
[`OBDS-1.1.5-CHANGELOG.md`](OBDS-1.1.5-CHANGELOG.md) for what moved and what did
not, [`OBDS-1.1.5-MIGRATION.md`](OBDS-1.1.5-MIGRATION.md) if you are on an
earlier release.
...
across 1.0.0, 1.0.1, 1.0.2, 1.0.3, 1.0.4, 1.1.0 and 1.1.1, and the release gate fails
if that ever stops being true. Every one of their `$id` values still resolves
under `/schemas/1.0.0/` and `/value-schemas/1.0.0/`.

OBDS 1.1 adds exactly one versioned contract beside them and changes none of
them:
[`schemas/1.1.0/compiled-context.schema.json`](schemas/1.1.0/compiled-context.schema.json),
which adds `governedResultHash` and pins `schemaVersion` to `1.1.0`. A 1.1
implementation reads 1.0.0 manifests and produces 1.1.0 compiled contexts. A
strict 1.0.0 consumer will reject a 1.1.0 compiled context, which is what the
new contract version is for.
...
Seven suites: foundation 104, context-delivery 3, context-assembly 15,
design-space 18, integration 15, golden 6, adversarial 38. No `skipif`, no
...
Its result is published as `OBDS-1.1.5-FOUNDATION-CONFORMANCE.json`, and
`reference/release-gate.py` re-executes it on every run, so a release cannot
omit it.

**These two numbers are not added together.** Fourteen of the fifteen declared
...
199 would double-count the same coverage. The one case that is not duplicated,
`canonical-hashes`, pins the canonical hashing of the published
...
modules other than the reference compiler. `OBDS-1.1.5-TEST-RESULT.json`
...
OBDS 1.1.1, stable, 30 August 2026. Published at
