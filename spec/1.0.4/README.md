...
| [`OBDS-1.0.4-IMPLEMENTER-QUICKSTART.md`](OBDS-1.0.4-IMPLEMENTER-QUICKSTART.md) | five concepts, then the smallest conforming implementation |
| [`OBDS-1.0.4.md`](OBDS-1.0.4.md) | the normative specification |
...
| [`OBDS-1.0.4-TEST-RESULT.json`](OBDS-1.0.4-TEST-RESULT.json) | the verified conformance result |

Also: [`CONTRIBUTING.md`](CONTRIBUTING.md) for how to propose a change,
[`OBDS-1.0.4-CHANGELOG.md`](OBDS-1.0.4-CHANGELOG.md) for what moved and what did
not, [`OBDS-1.0.4-MIGRATION.md`](OBDS-1.0.4-MIGRATION.md) if you are on an
earlier release.

### `schemaVersion: 1.0.0` in a 1.0.4 release

Not a mistake, and not a typo to work around. The release version and the schema
contract version are deliberately separate. 1.0.4 changes documentation,
...
contracts are byte-identical across 1.0.0, 1.0.1, 1.0.2, 1.0.3 and 1.0.4, and the
...
Its result is published as `OBDS-1.0.4-FOUNDATION-CONFORMANCE.json`, and
...
107 would double-count the same coverage. The one case that is not duplicated,
...
modules other than the reference compiler. `OBDS-1.0.4-TEST-RESULT.json`
...
OBDS 1.0.4, stable, 29 August 2026. Published at
