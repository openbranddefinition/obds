...
status         ready
artifactRef    generations/6e051b97cd804fee753a1e71ccc695a0a00d936d08b789f8bc193c18e81b1a06/target-bea1c8bf6e2f0c8c990e7ec685b2f67a98bc8fe1111aaaae278176477cb8f123.context.json
...
/tmp/obds-out/build-report.yaml
/tmp/obds-out/generations/6e051b97cd804fee753a1e71ccc695a0a00d936d08b789f8bc193c18e81b1a06/target-bea1c8bf6e2f0c8c990e7ec685b2f67a98bc8fe1111aaaae278176477cb8f123.context.json
(the same generation also contains its report and Markdown view)
```
...
/tmp/obds-fail build-report.yaml and generations/<digest>/build-report.yaml
              (no Compiled Brand Context for this failed generation)
```
...
| [`OBDS-4.1.3-IMPLEMENTER-QUICKSTART.md`](OBDS-4.1.3-IMPLEMENTER-QUICKSTART.md) | five concepts, then the smallest conforming implementation |
| [`OBDS-4.1.3.md`](OBDS-4.1.3.md) | the normative specification |
...
| [`OBDS-4.1.3-TEST-RESULT.json`](OBDS-4.1.3-TEST-RESULT.json) | the verified conformance result |

Also: [`CONTRIBUTING.md`](CONTRIBUTING.md) for how to propose a change,
[`OBDS-4.1.3-CHANGELOG.md`](OBDS-4.1.3-CHANGELOG.md) for what moved and what did
not, [`OBDS-4.1.3-MIGRATION.md`](OBDS-4.1.3-MIGRATION.md) if you are on an
earlier release.
...
that is deliberate. The 27 public OBDS 1.0.0 contracts have been byte-identical
in every release since 1.0.0, this one included, and the release gate fails if
that ever stops being true. The `notes` of
[`OBDS-4.1.3-TEST-RESULT.json`](OBDS-4.1.3-TEST-RESULT.json) list every earlier
release the identity is proven against. Every one of their `$id` values still resolves
...
OBDS 4.0 adds three contracts under [`schemas/4.0.0/`](schemas/4.0.0/): Model Input Package with reproducible projection metadata, Build Report with immutable generation binding, and Runtime Decision Record with `model_failed`. Build Plan and Compiled Brand Context retain their 3.0.0 contracts.
...
Seven suites: foundation 1052, context-delivery 3, context-assembly 24,
...
Its result is published as `OBDS-4.1.3-FOUNDATION-CONFORMANCE.json`, and
...
modules other than the reference compiler. `OBDS-4.1.3-TEST-RESULT.json`
...
OBDS 4.1.3, stable, 12 September 2026. Published at
