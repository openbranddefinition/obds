...
The published OBDS materials are the specification, the schemas, the value schemas, the tests, the reference implementation and the other original material in the release package.

Anyone may read, study and evaluate them. Free use covers personal use, education and teaching, academic research, non-profit research, testing and experimentation, evaluation, and proof-of-concept work that neither runs in production nor is delivered to a paying client. Internal evaluation by a commercial organisation is free use under that same condition.

A separate commercial licence from the publisher, Kill The Dragon GmbH, is required for paid client work, agency services, consulting using OBDS, production use by a commercial organisation, SaaS or managed services based on OBDS, embedding OBDS in a commercial product, commercial redistribution, and selling OBDS-based implementations, workflows or deliverables.

This licensing applies to the published materials. It does not claim exclusive ownership of the underlying ideas, principles or methods, and it does not prevent anyone from building a different system from similar ideas without copying or using OBDS materials.

The names Open Brand Definition and OBDS, together with the associated logos and marks, are not granted under any content or software licence. Trademark rights are reserved separately.

The current licence terms and the licensing contact are published at `https://openbranddefinition.org`.
...
1. one normative specification: `OBDS-1.0.1.md`;
