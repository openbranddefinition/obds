    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        for kind in ['SoftwareSourceCode', 'TechArticle']:
            def stale(document, kind=kind):
                next(node for node in document['@graph'] if node.get('@type') == kind)['version'] = '4.0.4'
        ...
        def stale_download(document):
            next(node for node in document['@graph'] if node.get('@type') == 'SoftwareSourceCode')['url'] = 'https://openbranddefinition.org/spec/4.0.4/OBDS-4.0.4-FINAL.zip'
        ...
        trial('companion about stale', companion,
              structured(companion, lambda document: document['about'].update(version='4.0.4')))
        trial('companion decoy current version cannot satisfy about', companion,
              structured(companion, lambda document: (document['about'].update(version='4.0.4'), document.update(version='4.1.0'))))
        for rel in [homepage, companion]:
            ...
            trial(rel + ' duplicate JSON key', rel, original.replace('"version": "4.1.0"', '"version": "4.0.4", "version": "4.1.0"', 1))
        ...
        trial('duplicate identity in separate script', homepage,
              originals[homepage].replace('</head>', '<script type="application/ld+json">{"@id":"https://openbranddefinition.org/#implementation","@type":"SoftwareSourceCode","version":"4.1.0"}</script></head>'))
        status = '<div class="status">OBDS / 4.1.0 stable</div>'
        stale_status = originals[homepage].replace(status, status.replace('4.1.0', '4.0.4'), 1)
        ...
        assert '4.0.4' in originals['llms.txt']
        trial('existing intentional 4.0.4 history remains valid', accepted=True)
        current_line = 'Current release: 4.1.0 (stable, 9 September 2026)'
        trial('llms stale declaration plus current copy in other section', 'llms.txt',
              originals['llms.txt'].replace(current_line, current_line.replace('4.1.0', '4.0.4'), 1) + '\n' + current_line + '\n')
        anchor = '<a href="/spec/4.1.0/OBDS-4.1.0.md">'
        ...
        for index, offset in enumerate(positions):
            ...
            replacement = original[:offset] + anchor.replace('4.1.0', '4.0.4') + original[offset + len(anchor):]
...
def verify_repository_gate_closure():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        for rel in ['OBDS-4.1.0-CHANGELOG.md', 'OBDS-4.1.0-IMPLEMENTER-QUICKSTART.md',
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        for phrase in gate.RETIRED_LICENSING_WORDING:
            ...
            trial('unclassified preamble rejects ' + phrase,
                  prefix.replace(b'## 4.1.0', encoded + b'\n\n## 4.1.0', 1) + history, wording=True)
            trial('verified historical heading cannot shelter new ' + phrase,
                  prefix + history.replace(b'## 1.0.2\n', b'## 1.0.2\n' + encoded + b'\n', 1))
            for rel in ['README.md', 'OBDS-4.1.0-IMPLEMENTER-QUICKSTART.md', *gate.PUBLICATION_URLS]:
                base = (ROOT / rel).read_bytes()
                trial(rel + ' intact current surface rejects ' + phrase,
                      base + b'\n' + encoded + b'\n', rel=rel, wording=True)
                (site / rel).write_bytes(base)
        for heading in [b'', b'## 4.1.0\n\n## 4.1.0', b'### 4.1.0', b'##4.1.0',
                        b'## 4.1', b'## 4.1.0 trailing', b'## 4.1.0 ##', b' ## 4.1.0',
                        b'## 4.0.4', b'## 4.2.0', b'4.1.0\n------']:
            trial('missing/malformed/conflicting current heading ' + repr(heading),
                  prefix.replace(b'## 4.1.0', heading, 1) + history)
        for label, mutation in [
            ...
            ('malformed historical title', original.replace(b'# OBDS 4.0.4 -', b'## OBDS 4.0.4 -', 1)),
...
def verify_licensing_test_source():
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        for rel in ['tools/test-other.py', 'tests/test-other.py', 'tools/README.md',
                    ...
                    'OBDS-4.1.0-IMPLEMENTER-QUICKSTART.md', *gate.PUBLICATION_URLS]:
