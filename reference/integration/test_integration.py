...
def test_single_normative_spec_no_core_file():
    assert (ROOT/'OBDS-1.1.6.md').exists()
...
def test_schema_index_capabilities_and_files_are_synchronised():
    registry=json.loads((ROOT/'OBDS-1.1.6-CAPABILITY-REGISTRY.json').read_text())
    index=json.loads((ROOT/'OBDS-1.1.6-SCHEMA-INDEX.json').read_text())
    allowed={'foundation'}
