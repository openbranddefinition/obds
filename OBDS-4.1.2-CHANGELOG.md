...
Prospective MINOR from 4.0.4: one voluntary normative Task Facts 1.0 evaluator capability (`task-facts`, section 26.10), complete adoption wrapper, preserved schema provenance/payload 0.1/TFJ-0.1, separate language-neutral suite and complete evidence bundle. Existing acceptance rules and governed hashes are unchanged. Separate research Python and Node subjects each run 66 fixture decisions and 36 regression records; probes are supplementary evidence. Packaging adds deterministic archives and complete current-release publication checks. No Core runtime integration, rights/legal verification, independent certification, cross-model benchmark or production readiness claim.

# OBDS 4.0.4 - Public surface
...
OBDS 4.0.4 is a public-surface release. It gives every public page its own Open Graph preview image and repairs a grid defect that let the German copy push the page wider than the viewport. No normative OBDS contract, schema, runtime behaviour, conformance behaviour or interoperability contract changed.

### What moved

- Open Graph images. Every public page carried `og:title` and `og:description` but no
  `og:image`, and `twitter:card` was `summary`, the small card without a picture. Each of the
  eight pages now has a 1200x630 card of its own at `/og/<slug>.png`, declared with its width,
  height, type and alt text, and `twitter:card` is `summary_large_image`.

  The cards are generated by `tools/build-og-images.py`, which reads the title from the page's
  own `og:title` rather than from a second list, so a card cannot drift from the page it
  belongs to. They are set in the site's own vocabulary: black paper, white ink, Helvetica
  Neue for the title, monospace for the machine-facing lines, and the frame and letterforms of
  `favicon.svg` as the mark. The generator lives in `tools/`, which the site never serves.

- Grid overflow. `1fr` is the short form of `minmax(auto, 1fr)`, so a grid column cannot
  shrink below the width of the longest word inside it. German sets longer words than English:
  `MASCHINENLESBARE`, set in the display grade of the four-column action grid, made those four
  columns wider than the viewport, and the grid was cut off at the right edge. English did not
  show it because the words are shorter.

  Every `repeat(N, 1fr)` is now `repeat(N, minmax(0, 1fr))`, 54 of them across the eight
  pages, and `body` carries `overflow-wrap: break-word` so a word that still cannot fit breaks
  instead of leaving its column. Before the change 46 grid definitions in `index.html` had a
  single `minmax(0, ...)` between them; any long German heading would have broken the same way.

- Previous release pointer. The publication block on the homepage named OBDS 3.0.4 as the
  previous release, which had been true before 4.0.0 shipped. It names 4.0.3.

- `llms.txt`: the previous-release list skipped 4.0.0, 4.0.1, 4.0.2 and 4.0.3 while claiming to
  be complete. All four are listed.

### What did not move

No schema, no reference implementation source, no test assertion about OBDS behaviour, no
conformance case and no historical release. `spec/4.0.3/`, `spec/4.0.2/`, `spec/4.0.1/` and
`spec/4.0.0/` are byte-identical and their tags are unchanged.

### Why this is a release and not a patch to the site

`examples/index.html` is part of the published release archive, not only of the website. The
release gate refuses a working tree whose file hashes disagree with `PACKAGE-MANIFEST.json`,
and rebuilding the 4.0.3 archive in place would overwrite a published release. Changing that
file therefore cuts a new version, which is what this is.

### Open

The homepage still has no dedicated German URL. Unchanged from 4.0.3 and recorded in the
migration notes with its cost.
...
**Documentation and public-surface neutrality patch. PATCH.**

OBDS 3.0.3 is a documentation and public-surface neutrality patch.

It replaces one sector-specific example value with a neutral placeholder.
...
Section 9, the Scope example:

```yaml
  productFamilies: [example-family]
```

The previous value was a sector-specific product-family name.

`productFamilies` remains one of the nine dimensions of the closed Scope
vocabulary. The example value is an opaque identifier in both spellings, so
nothing about scope construction, applicability, precedence, selection or
conflict relevance is affected. The paragraph below the block that declares the
vocabulary closed is unchanged.

The reason is presentation, not correctness. The previous value named an
industry in a document that has no reason to name one, and the public research
surface published with this release is required to be sector-neutral.

### Contract identity vs 3.0.2
...
Only the release version stamp, the release-file name, the specification's own
document hash and the archive hash move, and they move because a file in the
package is four bytes longer.

3.0.2 is not withdrawn and is not defective. Its published bytes, hashes,
archive and tag are unchanged and remain reproducible.

### Also in this release

The Research Package is published under `research/`: the deterministic
governed-result proof, the Governed Communications Benchmark, and the stated
limits of the mechanism. It is documentation and evidence. It adds no
specification content, no schema and no runtime behaviour, and the release gate
treats it as it treats any other non-package directory.
